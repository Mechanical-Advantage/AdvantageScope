...
export default class LogExporter {
  static async generateBin(log: Log, options: ExportOptions, progress: (progress: number) => void = (progress) => { }): Promise<Uint8Array | string> {
    // return "";
    ...
    switch (options.format) {
      case "csv-table":
        return LogExporter.generateCsvTable(log, fields, progress, timestamps);
      case "csv-list":
        return LogExporter.generateCsvList(log, fields, progress, timestamps);
      case "wpilog":
        return LogExporter.generateWPILOG(log, fields, progress, timestamps);
      case "mcap":
        return await LogExporter.generateMCAP(log, fields, progress, timestamps);
    }
