  ...
  */
  ...
  getDataInRange(fieldIndex, startTime, endTime) {
    ...
    if ("arrayParent" in field) {
      ...
      return {
        ...
        values: parentData.values.map((value) =>
          value == null || field.arrayIndex >= value.length ? null : value[field.arrayIndex]
        ),
    ...
    var startValueIndex = field.timestampIndexes.findIndex(
      (timestampIndex) => this.#timestamps[timestampIndex] > startTime
    );
    ...
    var endValueIndex = field.timestampIndexes.findIndex(
      (timestampIndex) => this.#timestamps[timestampIndex] >= endTime
    );
    if (endValueIndex == -1 || endValueIndex == field.timestampIndexes.length - 1) {
  ...
  add(entry) {
    ...
    for (let i in entry.data) {
      if (entry.data[i].type == "null") {
        // Set all types if null
        ...
        // Not null, update the data normally
        var fieldIndex = this.#fields.find(
          (field) => field.key == entry.data[i].key && field.type == entry.data[i].type
        );
        fieldIndex = this.#fields.indexOf(fieldIndex);
        if (fieldIndex > -1) {
          // Field already exists, add to it
    ...
    for (let i in this.#fields) {
      ...
      if (!allKeys.includes(this.#fields[i].key)) {
        ...
        if (duplicateIndexes.length > 1) {
          // Duplicate key, use type identifier
          for (let x in duplicateIndexes) {
            this.#fields[duplicateIndexes[x]].displayKey =
              this.#fields[duplicateIndexes[x]].key + "[" + this.#fields[duplicateIndexes[x]].type + "]";
          }
        } else {
          // Unique key
