...
export class SideBar {
      ...
      if (field.field != null) {
        ...
        window.addEventListener("mousemove", (event) => {
          if (mouseDownInfo != null) {
            if (
              Math.abs(event.clientX - mouseDownInfo[0]) >= this.#fieldDragThreshold ||
              Math.abs(event.clientY - mouseDownInfo[1]) >= this.#fieldDragThreshold
            ) {
  ...
  updateTitle() {
    ...
    this.#sideBarTitle.innerText =
      log.getFieldCount(false).toString() +
      " field" +
      (log.getFieldCount(false) == 1 ? "" : "s") +
      ", " +
