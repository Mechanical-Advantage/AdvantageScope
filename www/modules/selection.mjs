...
export class Selection {
  ...
  constructor() {
    ...
    this.#unlockedButton.addEventListener("click", () => {
      this.lock();
    });
    this.#lockedButton.addEventListener("click", () => {
      this.unlock();
    });
    window.addEventListener("keydown", (event) => {
    ...
    var setPlaybackSpeed = () => {
      window.dispatchEvent(
        new CustomEvent("set-playback-speed", {
  ...
  get selectedTime() {
    if (window.liveStatus == 2 && this.#locked) {
      // Based on live logging start time (continuously increase regardless of data)
      return new Date().getTime() / 1000 - window.liveStart + log.getTimestamps()[0];
    } else if (this.#playing) {
      // Increase from last selected time until reaching the end of data
      var time =
        (new Date().getTime() / 1000 - this.#playStart) * this.#playbackSpeed +
        (this.#selectedTime == null ? 0 : this.#selectedTime);
