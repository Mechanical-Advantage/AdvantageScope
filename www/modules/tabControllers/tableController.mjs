...
export class TableController {
  ...
  get state() {
    ...
    var time =
      log == null
        ? null
        : log.getTimestamps()[Math.floor(this.#tableContainer.scrollTop / this.#rowHeight) + this.#currentRange[0]];
  ...
  set state(newState) {
    ...
    this.#currentRange = [
      0,
      log.getTimestamps().length < this.#maxRows ? log.getTimestamps().length - 1 : this.#maxRows - 1
    ];
  ...
  #jumpToTime(targetTime) {
    ...
    var target = log
      .getTimestamps()
      .findIndex((value) => Math.floor(value * this.#maxRows) / this.#maxRows > targetTime);
    ...
    if (log.getTimestamps().length < this.#maxRows) {
      ...
      if (this.#currentRange[1] > log.getTimestamps().length - 1)
        offset = log.getTimestamps().length - 1 - this.#currentRange[1];
    ...
    var highlight = (time, className) => {
      ...
      if (time) {
        ...
        if (target >= this.#currentRange[0] && target <= this.#currentRange[1])
          this.#tableBody.children[target - this.#currentRange[0] + 1].classList.add(className);
      }
    };
  ...
  periodic() {
    ...
    if (!atMaxRows) {
      ...
      if (
        this.#tableContainer.scrollHeight - this.#tableContainer.clientHeight - this.#tableContainer.scrollTop <
          this.#scrollMargin &&
        this.#currentRange[1] < log.getTimestamps().length - 1
      ) {
        offset =
          this.#scrollMargin -
          (this.#tableContainer.scrollHeight - this.#tableContainer.clientHeight - this.#tableContainer.scrollTop);
    ...
    if (rowOffset != 0) {
      ...
      if (this.#currentRange[1] + rowOffset > log.getTimestamps().length - 1)
        rowOffset = log.getTimestamps().length - 1 - this.#currentRange[1];
      if (atMaxRows) {
