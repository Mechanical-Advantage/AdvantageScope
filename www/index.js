...
window.addEventListener("start-live", (event) => {
  ...
  decodeWorker.onmessage = (event) => {
    switch (event.data.status) {
      ...
      case "newLiveData": // New data to show
        ...
        var timeRange =
          window.log.getTimestamps()[window.log.getTimestamps().length - 1] - window.log.getTimestamps()[0];
        ...
        if (firstData) {
          ...
          setLiveStatus(2);
        } else if (window.log.getFieldCount(true) != oldFieldCount) {
          // Reset state when fields update
  ...
  // Start!
  window.dispatchEvent(
    new CustomEvent("start-live-socket", {
      detail: {
    ...
    window.dispatchEvent(
      new CustomEvent("error", {
        detail: {
          ...
          content: "Could not connect to log server at " + window.liveAddress + ":" + prefs.port
        }
      })
    );
...
window.addEventListener("export-csv", () => {
  if (log == null || window.liveStatus != 0) {
    ...
    window.dispatchEvent(
      new CustomEvent("export-csv-dialog", {
