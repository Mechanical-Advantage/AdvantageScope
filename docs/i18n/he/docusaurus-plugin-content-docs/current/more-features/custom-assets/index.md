...
תיקייה זו צריכה להכיל קובץ בשם "config.json" וקובץ נכס אחד או יותר, כפי שמתואר למטה. קובץ התצורה כולל תמיד את שם הנכס להצגה על ידי AdvantageScope. שם זה חייב להיות ייחודי עבור כל סוג נכס. נכסים יכולים לכלול באופן אופציונלי גם אובייקט "locales" הממפה קודי שפה (למשל "en-US", "es-419", "fr") לשמות מתורגמים. ה-"name" ברמת השורש נחשב כשם ברירת המחדל אם השפה שנבחרה אינה מסופקת או ש-"locales" הושמט.

```json
{
  "name": string, // שם ברירת מחדל ייחודי, נדרש עבור כל סוגי הנכסים
  "locales": { [locale: string]: string } // שמות מקומיים אופציונליים הממפים קודי שפה (למשל "en-US", "fr") לתרגומים
  ... // תצורה תלוית-סוג, מתוארת למטה
}
```

## מודלי רובוט 3D {#3d-robot-models}

### מדריך וידאו {#video-tutorial}
...
```json
{
  ...
  "locales": { [locale: string]: string } // תרגומים אופציונליים לשם הנכס
  ...
  "cameras": [ // מיקומי מצלמה קבועים, יכול להיות ריק
    {
      ...
      "locales": { [locale: string]: string } // תרגומים אופציונליים לשם המצלמה
      "rotations": { "axis": "x" | "y" | "z", "degrees": number }[] // רצף סיבובים לאורך צירי x, y ו-z
...
### רכיבים מפרקיים {#articulated-components}
...
:::info
לחצנים, ג'וייסטיקים וערכי צירים תומכים הן בקישורי [SDL](https://www.libsdl.org) (המשמשים את עמדת הנהגים הנוכחית ב-FIRST Driver Station) והן בקישורי NI (המשמשים את עמדת הנהגים הישנה ב-NI FRC Driver Station). לפחות סט קישורים אחד חייב להיות מסופק עבור כל רכיב.
...
### לחצן יחיד / ערך POV {#single-button-pov-value}
...
### בקר דו-צירי (Joystick) {#two-axis-joystick}
...
## תמונות מגרש שטוחות {#flat-field-images}
...
## מודלי מגרש 3D {#3d-field-models}

מודל חייב להיכלל בתיקייה עם השם "model.glb". לאחר הפעלת כל הסיבובים, המגרש צריך להיות מופנה כאשר הברית האדומה משמאל. קובצי CAD חייבים להמיר ל-glTF; ראו [דף זה](gltf-convert) לפרטים. מודלי אביזר משחק עוקבים אחר מוסכמת מתן השמות "model_INDEX.glb" בהתבסס על הסדר שבו הם מופיעים במערך "gamePieces". AprilTags שמוצהרים כאן ממוקמים תמיד תוך שימוש במערכת צירים [מרכז/אדום](/more-features/coordinate-systems#center-red), ללא קשר לאפשרויות תצורה אחרות.
  ...
  "gamePieces": [ // רשימת סוגי אלמנטי משחק
    {
      ...
      "locales": { [locale: string]: string } // תרגומים אופציונליים לשם אלמנט המשחק
      "rotations": { "axis": "x" | "y" | "z", "degrees": number }[] // רצף סיבובים לאורך צירי x, y ו-z
      "position": [number, number, number] // היסט מיקום במטרים, מופעל לאחר סיבוב
      "stagedObjects": string[] // שמות אובייקטי אביזר משחק ערוכים, להסתרה אם מסופקות תנוחות משתמש
    },
    ...
  ],
