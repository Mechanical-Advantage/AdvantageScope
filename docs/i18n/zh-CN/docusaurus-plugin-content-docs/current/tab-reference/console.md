...
此处的控件类似于 🔢 [表格](../tab-reference/table) 选项卡。所选时间将在所有选项卡中同步。点击某行即可选中它，或者将鼠标悬停在某行上可在任何可见的弹出窗口中进行预览。点击 ↓ 按钮可跳转到所选时间（或在框中输入的指定时间）。时间戳和跳转输入将根据 [时间戳](/more-features/timestamps) 偏好设置进行格式化。

在“过滤”输入框中输入文本可仅显示包含过滤文本的行。按 `Ctrl+F` 可快速选中“过滤”输入框。在过滤文本的开头添加 "!" 可从主视图中 _排除_ 匹配的消息。

## ANSI 格式化 {#ansi-formatting}

控制台渲染器支持使用标准 ANSI 转义序列进行格式化和颜色编码：

- **文本样式：** 粗体（bold）、变暗（dim）、斜体（italic）和下划线（underline）
- **前景色和背景色：** 16 种标准颜色（标准和高亮度）
- **8 位和 24 位颜色：** 256 色查找调色板和 24 位 RGB
- **选择性重置：** 重置特定样式或颜色，同时保留其他样式
