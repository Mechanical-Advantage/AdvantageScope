...
:::tip
如果单位解析错误，点击 `手动单位` > `禁用自动单位` 以忽略单位信息。然后可以使用手动转换切换到其他单位。
:::
...
要配置手动转换，请点击轴标题附近的三个点，然后点击 `手动单位` > `编辑换算...`。选择单位类型、原单位和目标单位。每个值还会乘以“额外缩放系数”，从而允许自定义转换（如齿轮比、角量到线量的转换，或 AdvantageScope 未提供的其他单位）。因数也可以使用数学表达式输入，例如 `1.5*pi`。
