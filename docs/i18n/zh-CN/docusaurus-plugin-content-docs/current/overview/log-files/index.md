...
:::info
如果同时打开多个文件，时间戳将自动对齐。这可以轻松比较来自多个数据源的日志文件。有关时间戳显示选项的详细信息，请参阅 [时间戳](/more-features/timestamps) 页面。
:::
...
点击 `文件` > `下载日志...` 打开下载窗口。Systemcore 和 roboRIO 支持日志下载。连接到机器人后，可用的日志将显示在列表中，最新日志位于顶部。选择一个或多个要下载的日志文件或文件夹（按住 shift 点击选择范围或使用 **cmd/ctrl + A** 全选）。然后点击 ↓ 符号并选择保存位置。
