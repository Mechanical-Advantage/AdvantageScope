...
当连接到实时数据源时，AdvantageScope 默认将所有选项卡锁定到当前时间。诸如 📉 [折线图](/tab-reference/line-graph) 和 🔢 [表格](/tab-reference/table) 等视图会自动滚动，而场地和操纵杆等视图则显示每个字段的当前值。点击导航栏中的红色箭头按钮可以切换此锁定，从而允许查看和重放过去的数据。
