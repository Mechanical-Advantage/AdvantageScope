...
:::info
إذا تم فتح ملفات متعددة في وقت واحد، سيتم محاذاة الطوابع الزمنية تلقائياً. يتيح ذلك سهولة مقارنة ملفات السجل من مصادر متعددة. راجع صفحة [الطوابع الزمنية](/more-features/timestamps) للحصول على تفاصيل حول خيارات عرض الطوابع الزمنية.
:::
...
بعد فتح ملف سجل، يمكن بسهولة إضافة سجلات إضافية إلى التصور المرئي. ستتم إعادة محاذاة الطوابع الزمنية تلقائياً للمزامنة مع البيانات الحالية.

في شريط القوائم، انقر فوق `ملف` > `إضافة سجل جديد...`، ثم اختر ملف سجل واحد أو أكثر لإضافته إلى التصور المرئي الحالي. سيتم تسجيل الحقول من كل سجل تحت جداول مسماة `Log0` و `Log1`، وما إلى ذلك.
...
افتح نافذة التفضيلات بالنقر فوق `التطبيق` > `عرض التفضيلات...` (Windows/Linux) أو `AdvantageScope` > `الإعدادات...` (macOS). قم بتحديث عنوان الروبوت ومجلد السجلات.

<img src="/img/prefs_ar.webp" alt="مخطط التفضيلات" height="450" />
</details>

انقر فوق `ملف` > `تنزيل السجلات...` لفتح نافذة التنزيل. تنزيل السجلات مدعوم على Systemcore و roboRIO. بمجرد الاتصال بالروبوت، يتم عرض السجلات المتاحة مع أحدثها في الأعلى. حدد ملف سجل واحد أو أكثر لتنزيله (انقر مع الضغط على shift لتحديد نطاق أو **cmd/ctrl + A** لتحديد الكل). ثم انقر فوق الرمز ↓ وحدد موقع الحفظ.

:::info
يستخدم [مسجل الإشارات](https://v6.docs.ctr-electronics.com/en/stable/docs/api-reference/api-usage/signal-logging.html) من CTRE تنسيقاً غير قياسي يقوم بتجميع السجلات في مجلدات فرعية. حدد مجلداً أو أكثر في القائمة لتنزيل ملفات السجل كمجموعة.
:::

:::tip
عند تنزيل ملفات متعددة، يتخطى AdvantageScope أي ملف موجود بالفعل في المجلد الوجهة.
:::
