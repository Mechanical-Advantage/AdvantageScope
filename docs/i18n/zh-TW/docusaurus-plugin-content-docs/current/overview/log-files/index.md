...
- **NI 駕駛站日誌 (.dslog 與 .dsevents)** - 舊版，由 NI 的 [FRC Driver Station](https://docs.wpilib.org/zh-cn/stable/docs/software/driverstation/driver-station.html) (2010-2026) 產生。開啟任一日誌類型時，AdvantageScope 會自動搜尋對應的日誌檔案。
...
:::info
如果同時開啟多個檔案，時間戳記將會自動對齊。這使得能夠輕鬆比較來自多個來源的日誌檔案。請參閱 [時間戳記](/more-features/timestamps) 頁面以取得有關時間戳記顯示選項的詳細資訊。
:::
...
點擊 `檔案` > `下載日誌...` 開啟下載視窗。Systemcore 與 roboRIO 支援日誌下載。連線到機器人後，可用的日誌會顯示在清單中，最新的位於頂部。選擇一個或多個要下載的日誌檔案或資料夾（Shift + 點擊可選擇範圍，或 **Cmd/Ctrl + A** 全選）。然後點擊 ↓ 符號並選擇儲存位置。
